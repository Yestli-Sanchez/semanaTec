...
if __name__ == "__main__":
    ...
    print("Describing 'books' table:")
    spark.sql("DESCRIBE books").show(20)

    query = """
    SELECT title, authors, average_rating, ratings_count 
    FROM books 
    WHERE ratings_count > 1000000 
    ORDER BY average_rating DESC
    """
    df_popular_books = spark.sql(query)
    df_popular_books.show(20)

    df_popular_books.write.mode("overwrite").json("results")
    spark.stop()
